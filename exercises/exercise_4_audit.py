...
async def node_analyze(state):
    ...
    await audit(state, AuditEntry(
        ...
        decision="pending",
        reason=f"Analyzed PR: {a.summary[:100]}...",
        execution_time_ms=int((time.monotonic() - t0) * 1000),
    ))
    return {"analysis": a}
...
async def node_human_approval(state):
    ...
    # Bonus 4: Auto-edit — if human chooses 'edit', refine the review with LLM
    if resp.get("choice") == "edit" and resp.get("feedback"):
        llm = get_llm()
        refine_prompt = f"""
        The original review had confidence {a.confidence:.0%}:
        Summary: {a.summary}
        Comments: {chr(10).join(f"- {c.severity}: {c.body}" for c in a.comments)}
        
        Human feedback: {resp["feedback"]}
        
        Please refine this review incorporating the feedback. Maintain the same structure.
        """
        refined = await llm.with_structured_output(PRAnalysis).ainvoke(refine_prompt)
        state["analysis"] = refined  # Update analysis with refined version
        a = refined  # Update local reference
    ...
    await audit(state, AuditEntry(
        ...
        reason=resp.get("feedback") or f"Human chose {resp.get('choice')}",
        execution_time_ms=int((time.monotonic() - t0) * 1000),
    ))
...
async def node_commit(state):
    ...
    await audit(state, AuditEntry(
        ...
        decision=action,
        reason=f"Committed review: {action}",
        execution_time_ms=int((time.monotonic() - t0) * 1000),
    ))
...
async def node_auto_approve(state):
    ...
    await audit(state, AuditEntry(
        ...
        risk_level=risk_level_for(a.confidence),
        decision="auto",
        reason=f"Auto-approved and posted comment",
        execution_time_ms=int((time.monotonic() - t0) * 1000),
    ))
...
async def node_escalate(state):
    ...
    await audit(state, AuditEntry(
        ...
        risk_level=risk_level_for(a.confidence),
        decision="escalate",
        reason="Escalating for specific questions",
        execution_time_ms=int((time.monotonic() - t0) * 1000),
    ))
    ...
    await audit(state, AuditEntry(
        ...
        reviewer_id=os.environ.get("GITHUB_USER"),
        decision="escalate",
        reason=f"Received answers to {len(answers)} questions",
        execution_time_ms=int((time.monotonic() - t0) * 1000),
    ))
...
async def node_synthesize(state):
    ...
    await audit(state, AuditEntry(
        ...
        decision="pending",
        reason=f"Synthesized refined review with answers",
        execution_time_ms=int((time.monotonic() - t0) * 1000),
    ))
